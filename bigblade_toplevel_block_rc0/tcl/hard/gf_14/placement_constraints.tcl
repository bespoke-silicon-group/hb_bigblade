...
for {set i 0} {$i < 4} {incr i} {
  for {set j 0} {$j < 2} {incr j} {
    ...
    move_object       $clk_gen_cell -x $clk_x -y $clk_y
    set_attribute     $clk_gen_cell orientation "R0"
    set_fixed_objects $clk_gen_cell

    set                bp_cell [get_cells "core_complex_core_${row}__bp_${pos}__halfpod"]
    move_object       $bp_cell -x $bp_start_x -y $y_new
    set_attribute     $bp_cell orientation "R0"
    set_fixed_objects $bp_cell

    create_placement_blockage -name "pb_bp_block_west_${i}_${j}" -boundary "{{[expr $bp_start_x-4*[unit_width]] $y_new} {$bp_start_x [expr $y_new+3*$tile_height+2*0.5*$grid_height]}}"
    create_placement_blockage -name "pb_bp_block_east_${i}_${j}" -boundary "{{[expr $bp_start_x+$bp_actual_width] $y_new} {[expr $bp_start_x+$bp_actual_width+4*[unit_width]] [expr $y_new+3*$tile_height+2*0.5*$grid_height]}}"
    create_placement_blockage -name "pb_bp_clk_west_${i}_${j}" -boundary "{{[expr $clk_x-4*[unit_width]] $clk_y} {$clk_x [expr $clk_y+$clk_gen_height]}}"
    create_placement_blockage -name "pb_bp_clk_east_${i}_${j}" -boundary "{{[expr $clk_x+$clk_gen_width] $clk_y} {[expr $clk_x+$clk_gen_width+4*[unit_width]] [expr $clk_y+$clk_gen_height]}}"
  }
}
...
for {set i 0} {$i < 4} {incr i} {
  for {set j 0} {$j < 2} {incr j} {
    ...
    move_object       $clk_gen_cell -x $clk_x -y $clk_y
    set_attribute     $clk_gen_cell orientation "MY"
    set_fixed_objects $clk_gen_cell

    set                cgra_cell [get_cells "core_complex_core_${row}__cgra_${pos}__halfpod"]
    move_object       $cgra_cell -x $cgra_start_x -y $y_new
    set_attribute     $cgra_cell orientation "R0"
    set_fixed_objects $cgra_cell

    create_placement_blockage -name "pb_cgra_block_west_${i}_${j}" -boundary "{{[expr $cgra_start_x-4*[unit_width]] $y_new} {$cgra_start_x [expr $y_new+4*$tile_height+3*0.5*$grid_height]}}"
    create_placement_blockage -name "pb_cgra_block_east_${i}_${j}" -boundary "{{[expr $cgra_start_x+$cgra_actual_width] $y_new} {[expr $cgra_start_x+$cgra_actual_width+4*[unit_width]] [expr $y_new+4*$tile_height+3*0.5*$grid_height]}}"
    create_placement_blockage -name "pb_cgra_clk_west_${i}_${j}" -boundary "{{[expr $clk_x-$clk_gen_width-4*[unit_width]] $clk_y} {[expr $clk_x-$clk_gen_width] [expr $clk_y+$clk_gen_height]}}"
    create_placement_blockage -name "pb_cgra_clk_east_${i}_${j}" -boundary "{{$clk_x $clk_y} {[expr $clk_x+4*[unit_width]] [expr $clk_y+$clk_gen_height]}}"
  }
}
...
# mux placement
set mux_cell_list [list]
append_to_collection mux_cell_list [get_cells -hier -filter "full_name=~*mc_mux*BSG_DONT_TOUCH*"]
append_to_collection mux_cell_list [get_cells -hier -filter "full_name=~*noc_mem_mux*BSG_DONT_TOUCH*"]
set idx 0
...
set mux_cell_list [list]
append_to_collection mux_cell_list [get_cells -hier -filter "full_name=~*bp_mux*BSG_DONT_TOUCH*"]
append_to_collection mux_cell_list [get_cells -hier -filter "full_name=~*cgra_mux*BSG_DONT_TOUCH*"]
set idx 0
foreach_in_collection mux_cell $mux_cell_list {
  set_attribute     $mux_cell orientation R0
  move_object       $mux_cell \
      -x [expr $core_llx+[round_down_to_nearest [expr 5112.742+$idx*5] [unit_width]]] \
      -y [expr $core_lly+10*[unit_height]]
  set_fixed_objects $mux_cell
  incr idx
}
