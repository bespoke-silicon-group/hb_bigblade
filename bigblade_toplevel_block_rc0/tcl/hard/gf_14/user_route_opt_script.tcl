...
proc insert_buffers_on_propagated_nets {port cell num start_num prefix buffer} {
  ...
  for {set i $start_num} {$i < [expr $num+$start_num]} {incr i} {
    ...
    add_buffer_on_route -net_prefix $prefix -cell_prefix $prefix -repeater_distance_length_ratio 0.5 $curr_net $buffer
  }
}
  ...
  for {set j 0} {$j < 16} {incr j} {
    ...
    foreach {k} {"fwd" "rev"} {
      create_generated_clock \
          ...
          [get_pins "core_complex_core_${i}__podrow/hor_io_${k}_link_clk_o[${j}]"]
      ...
      bsg_async_two_clocks $pod_clk_period_ps [get_clocks "hor_link_${i}_${j}_${k}_clk"] [get_clocks "tag_clk"]
    }
  }
}
      ...
      for {set l 0} {$l < $end_idx} {incr l} {
        foreach {k} {"fwd" "rev"} {
          create_generated_clock \
              ...
              [get_pins "core_complex_core_${i}__${blk}_${j}__halfpod/io_${k}_link_clk_o[${l}]"]
          ...
          bsg_async_two_clocks $pod_clk_period_ps [get_clocks "${blk}_link_${i}_${j}_${k}_${l}_clk"] [get_clocks "tag_clk"]
        }
      }
    }
  }
