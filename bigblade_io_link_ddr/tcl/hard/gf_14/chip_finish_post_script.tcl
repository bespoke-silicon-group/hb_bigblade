...
redirect -append -file ${REPORTS_DIR}/io_link_i.max.interface_timing.rpt {report_timing -from [get_ports {io_link_v_i io_link_data_i}] -delay_type max -max_paths 10 -nosplit}
redirect -append -file ${REPORTS_DIR}/io_link_i.min.interface_timing.rpt {report_timing -from [get_ports {io_link_v_i io_link_data_i}] -delay_type min -max_paths 10 -nosplit}
redirect -append -file ${REPORTS_DIR}/io_link_o.max.interface_timing.rpt {report_timing -to [get_ports {io_link_v_o io_link_data_o}] -delay_type max -max_paths 10 -nosplit}
redirect -append -file ${REPORTS_DIR}/io_link_o.min.interface_timing.rpt {report_timing -to [get_ports {io_link_v_o io_link_data_o}] -delay_type min -max_paths 10 -nosplit}
