...
for {set i 0} {$i < 4} {incr i} {
  ...
  create_routing_blockage -layers [get_layers] -boundary "{{[expr $pod_row_start_x-0.5*$grid_width] $lly} {$pod_row_start_x [expr $lly+2]}}"
  create_routing_blockage -layers [get_layers] -boundary "{{[expr $pod_row_start_x-0.5*$grid_width] $ury} {$pod_row_start_x [expr $ury-2]}}"
}
...
for {set i 0} {$i < 4} {incr i} {
  ...
  # gap routing blockages
  create_routing_blockage -layers [get_layers] -boundary "{{[expr $cgra_start_x-0.5*$grid_width] $lly} {$cgra_start_x [expr $lly+2]}}"
  create_routing_blockage -layers [get_layers] -boundary "{{[expr $cgra_start_x-0.5*$grid_width] $ury} {$cgra_start_x [expr $ury-2]}}"
}
