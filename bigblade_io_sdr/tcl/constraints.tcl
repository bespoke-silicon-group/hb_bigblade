...
########################################
## Clock Setup
set up_clk_name "up_clk"
set up_clk_period_ps 1000
set up_clk_uncertainty_per 3.0
set up_clk_uncertainty_ps [expr min([expr ${up_clk_period_ps}*(${up_clk_uncertainty_per}/100.0)], 20)]

set up_input_delay_per  20.0
set up_input_delay_ps  [expr ${up_clk_period_ps}*(${up_input_delay_per}/100.0)]
set up_output_delay_per 20.0
set up_output_delay_ps [expr ${up_clk_period_ps}*(${up_output_delay_per}/100.0)]

set down_clk_name "down_clk"
set down_clk_period_ps 900
...
########################################
## Reg2Reg
create_clock -period ${up_clk_period_ps} -name ${up_clk_name} [get_ports "uplink_clk_i"]
set_clock_uncertainty ${up_clk_uncertainty_ps} [get_clocks ${up_clk_name}]
...
create_generated_clock -name "tkn_clk" -source [get_pins uplink/osdr_phy/clk_pa/clk_ff_n/data_o[0]] -edges {1 3 5} -edge_shift {0 0 0} [get_pins downlink/downstream/token_counter/count_o[0]]

########################################
## In2Reg
set driving_lib_cell $LIB_CELLS(invx2)
set up_input_pins [filter_collection [filter_collection [all_inputs] "name=~uplink*" ] "name!~*clk*"]
set down_input_pins [filter_collection [filter_collection [all_inputs] "name=~downstream*" ] "name!~*clk*"]
set link_input_pins [filter_collection [filter_collection [all_inputs] "name=~link*" ] "name!~*clk*"]
set_input_delay ${up_input_delay_ps} -clock ${up_clk_name} ${up_input_pins}
...
set up_output_pins [filter_collection [all_outputs] "name=~uplink*" ]
...
set_load [load_of [get_lib_pin */${load_lib_pin}]] ${up_output_pins}
