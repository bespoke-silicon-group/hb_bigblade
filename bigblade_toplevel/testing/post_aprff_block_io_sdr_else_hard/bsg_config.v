...
// Macro for qp path
// Macro for pod path
// Macro for vcache array path (QP ID, pod x, array idx, tile y, tile x)
// Macro for tile array path (QP ID, pod x, north/south, tile x)
...
config bsg_config;
  ...
  // instance `BSG_TILE_PATH     liblist `BSG_MANYCORE_TILE_LIBRARY_NAME `BSG_CHIP_LIBRARY_NAME;
  // instance `BSG_SUBARRAY_PATH liblist `BSG_MANYCORE_TILE_COMPUTE_ARRAY_LIBRARY_NAME `BSG_CHIP_LIBRARY_NAME;  
  // Assign instance of pod_row to netlist
  `include "swaplist.vh"
  ...
  // These do not have hardened modules inside of them. OK to leave as-is.
  ...
  // No hardening in any of these modules
